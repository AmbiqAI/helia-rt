...
| `ADD` / `SUB` / `MUL` | :white_check_mark: | :white_check_mark: | Broadcasting is optimized for rank ≤ 4 with every dimension pair equal or 1; higher ranks use Reference (FP32) or are rejected at prepare (FP16) |
| `HARD_SWISH` | :white_check_mark: | :white_check_mark: | |
| `MEAN` / `SUM` | :white_check_mark: | :white_check_mark: | Optimized for rank ≤ 4 with any axis set; higher ranks use Reference (FP32) or return `kTfLiteError` (FP16) |
