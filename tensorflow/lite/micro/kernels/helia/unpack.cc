...
TfLiteStatus UnpackPrepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  TF_LITE_ENSURE_EQ(context, params->num, data->count);
  ...
  TF_LITE_ENSURE_EQ(context, input->dims->data[data->axis], data->count);
...
TfLiteStatus UnpackEval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (input->type) {
    case kTfLiteFloat32: {
      const auto* op_data =
          static_cast<const HeliaDataMovementData*>(node->user_data);
      ...
      for (int i = 0; i < op_data->count; ++i) {
        outputs[i] =
            micro::GetTensorData<float>(micro::GetEvalOutput(context, node, i));
      }
      const auto status =
          arm_unpack_f32(micro::GetTensorData<float>(input), input->dims->size,
                         op_data->shape, op_data->axis, outputs);
      TF_LITE_ENSURE_EQ(context, status, ARM_CMSIS_NN_SUCCESS);
      return kTfLiteOk;
#else
      return UnpackImpl<float>(context, node, input, op_data->count,
                               op_data->axis);
#endif
    }
    case kTfLiteFloat16: {
      ...
      TF_LITE_ENSURE(context, outputs != nullptr);
      for (int i = 0; i < op_data->count; ++i) {
        outputs[i] = micro::GetTensorData<float16_t>(
            micro::GetEvalOutput(context, node, i));
      }
      const auto status = arm_unpack_f16(micro::GetTensorData<float16_t>(input),
                                         input->dims->size, op_data->shape,
                                         op_data->axis, outputs);
      TF_LITE_ENSURE_EQ(context, status, ARM_CMSIS_NN_SUCCESS);
      return kTfLiteOk;
#else
      return kTfLiteError;
...
TFLMRegistration Register_UNPACK() {
  return tflite::micro::RegisterOp(InitHeliaDataMovement, UnpackPrepare,
                                   UnpackEval);
}
