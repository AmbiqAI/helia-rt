  ...
  switch (output->type) {
    case kTfLiteFloat16: {
      ...
      if (data->broadcast_class == HeliaBroadcastClass::kSameShape) {
        ...
        if (arm_elementwise_add_broadcast_f16(
    ...
    case kTfLiteFloat32: {
      ...
      if (data->broadcast_class == HeliaBroadcastClass::kSameShape) {
        ...
        ArmPopulateCommonParams(tflite::micro::GetTensorShape(input1),
                                ...
                                tflite::micro::GetTensorShape(output),
                                &input1_dims, &input2_dims, &output_dims);
        if (arm_elementwise_add_broadcast_f32(
...
TfLiteStatus PrepareAdd(TfLiteContext* context, TfLiteNode* node) {
  ...
  TF_LITE_ENSURE_MSG(
      ...
      "Float16 ADD does not support these input shapes.");
...
TfLiteStatus EvalAdd(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (output->type == kTfLiteFloat32 || output->type == kTfLiteFloat16 || output->type == kTfLiteInt32) {
    TF_LITE_ENSURE_OK(
