...
# ------------------------- 2. Per-config filter --------------------------------
# Mirror ext_libs/helia.inc: f16 is built only for TARGET_ARCH=cortex-m55.
SYMBOLS=()
ENABLE_F16=0
...
echo "==> fp-probe ${LABEL}: ${#SYMBOLS[@]} symbols (f16 $([[ ${ENABLE_F16} -eq 1 ]] && echo enabled || echo disabled))"
printf '    %s\n' "${SYMBOLS[@]}"

# ------------------------- 3. Generate the probe -------------------------------
...
# ------------------------- 4. Ask make for the leg's flags ---------------------
...
# LLD and armlink defaults. If every variant fails, all logs are printed.
LINK_VARIANTS=()
case "${TOOLCHAIN}" in
  armclang)
...
# Any of the three linkers naming one of our probe symbols as unresolved is a
# conclusive finding; there is no point trying the remaining variants, and
# doing so only buries the useful line under the fallbacks' own noise.
...
if [[ "${link_ok}" -ne 1 ]]; then
  ...
  if [[ -n "${conclusive}" ]]; then
    echo "fp-probe: unresolved symbols reported by the linker -- a real defect in" >&2
    echo "fp-probe: the library, not a probe configuration issue:" >&2
    grep -E "${UNDEF_RE}" "${conclusive}" >&2
    echo "fp-probe: full log: ${conclusive}" >&2
  else
    echo "fp-probe: every link variant failed without naming an unresolved" >&2
    echo "fp-probe: symbol. Full logs follow." >&2
